...
public class ExceptionUtils {
  ...
  /**
   * Throws a PackagedRuntimeException with an {@link InterruptedException} as cause if current
   * thread is interrupted.
   *
   * @param message
   *          exception's message
   * @throws PackagedRuntimeException
   *           current thread is interrupted
   */
  public static void throwRuntimeIfInterrupted(String message) throws PackagedRuntimeException {
    if (Thread.currentThread().isInterrupted()) {
      throw new PackagedRuntimeException(new InterruptedException(message));
    }
  }
